<template>
    ...
    <div class="px-5 py-6 lg:py-14">
        <nav class="font-sans text-[#777]">
            <router-link to="/">Home</router-link> / Hair Care
        </nav>
        <h1 class="my-4 mb-12 text-4xl lg:text-6xl">Hair Care</h1>

        <div class="flex items-center justify-between text-sm w-[100%]">
            <!-- Results Count Display -->
            <div v-if="totalPages > 1" class="w-[45%] lg:text-lg">Showing {{ startResult }}-{{ endResult }} of {{ products.length }} results</div>

            <div v-else class="w-[45%] lg:text-lg">Showing  all {{ endResult }} results</div>
        ...
        <div class="flex flex-col items-center mb-10">
            ...
            <div class="grid grid-cols-2 gap-5 mt-7 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                <Products  
                v-for="(product, index) of paginatedProducts" 
                :key="index"
                :product="product"
                />
            </div>
        ...
        <ul class="flex mt-10 mb-2 space-x-2">
          ...
          class="px-[.95rem] py-[.45rem] bg-white border border-black cursor-pointer hover:text-white hover:bg-black">
          {{ page }}
        </li>
